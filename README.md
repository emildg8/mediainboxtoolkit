...
3. `.\Update-MediaInboxReviewCsvAutoDecide.ps1 -CsvPath <review.csv> -TorrentDirectory "C:\Users\<user>\Downloads"` → `*.auto.csv`  
   ...
   Если qBittorrent качает в один UNC (например `\\NAS\qBittorrent\...\downloads`), а в CSV уже пути после переноса в сортировку (`\\NAS\emildg8\Video\Sort\...`), задайте пару префиксов: **`-QbittorrentCsvSourcePrefix`** (корень, как в CSV) и **`-QbittorrentDownloadRootPrefix`** (корень каталога загрузок qBittorrent). Относительный хвост пути тогда подставляется под каталог загрузок для поиска в индексе API. То же через `MIT_QBIT_CSV_PREFIX` и `MIT_QBIT_DOWNLOAD_ROOT`. У других пользователей клиент не обязателен — достаточно папки с `.torrent`.
   **Спецвыпуски (без SxxEyy в имени файла):** скопируйте `media-library-layout.example.json` → `media-library-layout.local.json` (или укажите **`-MediaLibraryLayoutJson`**, либо переменную **`MIT_MEDIA_LIBRARY_JSON`**). В JSON задайте **`scanRoots`** — те папки под `\\NAS\…\Video`, где лежат каталоги сериалов (как на вашем NAS: Мультсериалы, Сериалы, Анимесериалы и т.д.), и при необходимости **`MIT_VIDEO_LIBRARY_ROOT`** / **`-MediaLibraryVideoRoot`** до каталога `Video`. При запуске строится индекс подпапок; для строк `REVIEW` без эпизода срабатывают **`explicitRules`** (regex имени файла + имени папки + `libraryRoot`). Для **Анимесериалов** при совпадении с **`ovaFilenameRegex`** путь ведёт в подпапку **`ovaSubfolderName`** (например `OVA`). Опциональный fuzzy по `Notes` (`fuzzyEnabled`, `fuzzyUniqueMinScore`) — только один явный победитель. Отключить весь блок «плоско в корень сериала»: **`-SkipCartoonSeriesRootFlat`**. В консоли смотрите **`MediaLibraryRoot`** и **`SeriesIndexEntries`** (должно быть > 0, если NAS доступен).
