...
3. `.\Update-MediaInboxReviewCsvAutoDecide.ps1 -CsvPath <review.csv> -TorrentDirectory "C:\Users\<user>\Downloads"` → `*.auto.csv`  
   ...
   Опционально **qBittorrent Web UI** (если **полный путь** `SourceFullPath` в CSV совпадает с путём на машине, где крутится клиент — чаще всего это не UNC NAS, а локальный каталог загрузок): тот же базовый URL, что в браузере, **без** завершающего слэша, например `https://хост:порт/qbittorrent`. Если логина нет — параметры учётки не указывайте; при самоподписанном HTTPS: `-QbittorrentSkipCertificateCheck`. Иначе: `-QbittorrentUsername` / `-QbittorrentPassword` или `MIT_QBIT_WEBUI`, `MIT_QBIT_USER`, `MIT_QBIT_PASS`. У других пользователей клиент не обязателен — достаточно папки с `.torrent`. Парсить профиль Rutracker по URL не нужно.
