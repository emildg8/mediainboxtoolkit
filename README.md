...
3. `.\Update-MediaInboxReviewCsvAutoDecide.ps1 -CsvPath <review.csv> -TorrentDirectory "C:\Users\<user>\Downloads"` → `*.auto.csv`  
   ...
   **Спецвыпуски (без SxxEyy в имени файла):** если для строки `REVIEW` не удаётся разобрать эпизод, но на NAS уже есть каталог сериала под `...\Video\cartoons\<имя>` (скрипт читает список подкаталогов при запуске — NAS должен быть доступен), для настроенных шоу файл может быть назначен **прямо в корень** этой папки (без `Season N`). Сейчас задано сопоставление Robot Chicken / «Робоцып» (имя файла с `Robot.Chicken`, папка по regex в `Get-FlatSpecialLibraryRules` внутри скрипта). Расширять — добавляя элементы в эту функцию. Отключить правило: `-SkipCartoonSeriesRootFlat`.
