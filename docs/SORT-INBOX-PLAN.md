...
- Учёт `series-aliases` / локальных overrides для «плохих» имён (в политике: **`classification.seriesQueryRewrites`** — regex→`seriesGuess` до TMDB, опционально **`matchField`**: `seriesGuess` / `basename` / `relativePath` / `seriesGuessOrBasename`, правила по порядку; **`classification.seriesFolderByTmdbTvId`** — имя папки по `tv id` после матча).
